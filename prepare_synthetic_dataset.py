...
rootutils.setup_root(__file__, indicator="trash", pythonpath=True)
...
def get_different_color(previous_colors, random_state, min_dist=50, max_count=20):
    ...
    color = random_state.randint(256)
    count = 0
    while np.any(np.abs(previous_colors - color) < min_dist) and count < max_count: # max_count times check if new_color has enough contrast
        count += 1
        color = random_state.randint(256)
    return color
# Color functions
...
def draw_multiple_polygons(
    ...
    for i in range(nb_polygons):
        ...
        # custom_background = generate_custom_background(image.shape, background_color,
        #                                                **extra)
        custom_background = generate_background(background_size=image.shape, min_kernel_size=kernel_boundaries[0],
                                                max_kernel_size= kernel_boundaries[1], random_state=random_state)
...
def draw_star(image, random_state, nb_branches=6):
    ...
    for i in range(1, num_branches + 1):
        col = get_random_color(background_color, random_state=random_state)
        cv.line(image, (points[0][0], points[0][1]),
...
def draw_checkerboard(image, random_state, max_rows=7, max_cols=7, transform_params=(0.05, 0.15)):
    ...
    alpha_affine = np.max(image.shape) * (transform_params[0]
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            colors[i * cols + j] = col
            # Fill the cell
            cv.fillConvexPoly(image, np.array([(warped_points[i * (cols + 1) + j, 0],
    ...
    for _ in range(nb_rows):
        ...
        col = get_random_color(background_color, random_state=random_state)
        cv.line(image, (warped_points[row_idx * (cols + 1) + col_idx1, 0],
    ...
    for _ in range(nb_cols):
        ...
        col = get_random_color(background_color, random_state=random_state)
        cv.line(image, (warped_points[row_idx1 * (cols + 1) + col_idx, 0],
...
def draw_stripes(
    image, random_state, max_nb_cols=13, min_width_ratio=0.04, transform_params=(0.1, 0.1)
):
    ...
    background_color = int(np.mean(image))
    # Create the grid
    board_size = (int(image.shape[0] * (1 + random_state.rand())),
    ...
    for i in range(col):
        ...
        cv.fillConvexPoly(image, np.array([(warped_points[i, 0],
                                          warped_points[i, 1]),
                                         (warped_points[i+1, 0],
                                          warped_points[i+1, 1]),
    ...
    for _ in range(nb_rows):
        ...
        color = get_random_color(background_color, random_state=random_state)
        cv.line(image, (warped_points[row_idx + col_idx1, 0],
    ...
    for _ in range(nb_cols):
        ...
        color = get_random_color(background_color, random_state=random_state)
        cv.line(image, (warped_points[col_idx, 0],
...
def draw_cube(
    image, random_state, min_size_ratio=0.2, scale_interval=(0.4, 0.6), trans_interval=(0.5, 0.2)
):
    ...
    trans = np.array([image.shape[1] * trans_interval[0] +
                      random_state.randint(-image.shape[1] * trans_interval[1],
                                           image.shape[1] * trans_interval[1]),
                      image.shape[0] * trans_interval[0] +
                      random_state.randint(-image.shape[0] * trans_interval[1],
                                           image.shape[0] * trans_interval[1]),
                      0])
    ...
    for i in [0, 1, 2]:
        cv.fillPoly(image, [cube[faces[i]].reshape((-1, 1, 2))],
                    col_face)
    ...
    for i in [0, 1, 2]:
        for j in [0, 1, 2, 3]:
            ...
            cv.line(image, (cube[faces[i][j], 0], cube[faces[i][j], 1]),
...
def generate_background(background_size, random_state, nb_blobs=100, min_rad_ratio=0.01,
    ...
    """
    image = np.zeros(background_size, dtype=np.uint8)
    dim = max(background_size)
    cv.randu(image, 0, 255)
    cv.threshold(image, random_state.randint(256), 255, cv.THRESH_BINARY, image)
    background_color = int(np.mean(image))
    blobs = np.concatenate([random_state.randint(0, background_size[1], size=(nb_blobs, 1)),
                            random_state.randint(0, background_size[0], size=(nb_blobs, 1))],
                           axis=1)
    for i in range(nb_blobs):
        col = get_random_color(background_color, random_state=random_state)
        cv.circle(image, (blobs[i][0], blobs[i][1]),
...
def generate_data(n, draw_fn, background_size, image_size, blur_size, seed):    
    random_state = np.random.RandomState(seed)
    
    # сгенерировать изображение с фоном
    image = generate_background(background_size, random_state=random_state)

    # на фоне нарисовать примитивы
    image, points = draw_fn(image, random_state=random_state)

    # Checking for blur param
    if blur_size:
        cv.GaussianBlur(image, (blur_size, blur_size), 0, image)
        
    image = cv.resize(image, image_size)
    return image, points, n
    

def save_data(data_batch, out_dir, max_workers=None):
    ...
    def save_one_item(item):
        image, points, n = item
        filename = str(n).zfill(6)
        
        image_path = images_dir / f"{filename}.jpg"
        points_path = points_dir / f"{filename}.npy"
        
        cv.imwrite(str(image_path), image, [cv.IMWRITE_JPEG_QUALITY, 92])
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        results = list(tqdm.tqdm(
            executor.map(save_one_item, data_batch),
    ...
    for name, size in cfg.primitives.items():
        ...
        generated_data = Parallel(
            ...
            delayed(generate_data)(
                ...
                cfg.blur_size,
                cfg.seed + n
            ) for n in tqdm.tqdm(range(size), desc=f"Generating {name}")
        )
        
        save_data(generated_data, out_dir)
